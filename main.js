...
function detectSpecs() {
    ...
    if (!hasGetUserMedia) {
        ...
        prompt.innerHTML = 'Hello! Please allow camera access :)';
        init();
    }
...
function init() {
    ...
    gui.add(params, 'mOpac', 0, 1).name('Opacity').onChange(onParamsChange);
    gui.add(params, 'wfOpac', 0, 0.3).name('Grid').onChange(onParamsChange);
...
function saveImage() {
    render();
    window.open(renderer.domElement.toDataURL("http://experiments.samantafluture.com/images/png"));
}
